...
)
{
  ...
  $root_blueprint     = "${config_root}/git/diamond"
  $root_maestro       = "${config_root}/git/maestro"
  $blueprint_runtime  = 'puppet/modules/runtime_project'
  $blueprint_manifest = 'puppet/manifests'
  ...
  $sync_modules = "rsync -a ${root_blueprint}/${blueprint_runtime} ${modules_dir}"
  ...
  $sync_repo = "rsync -a ${root_maestro}/${blueprint_manifest} ${repo_dir}"
  ...
  # only perform this work if there is a git repo
  $rakespec_source = "${root_blueprint}/${blueprint_runtime}/Rakefile"
